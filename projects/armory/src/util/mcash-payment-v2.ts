...
import { prepare } from '../rx-operators/prepare';
...
export interface MCASHPaymentResult {
  status: 'success' | 'rejected' | 'pending';
  popupWindowObject?: any; // 팝업 객체 (status == pending일 때만 유효)
  form?: HTMLFormElement; // Form 태그 객체 (status == pending일 때만 유효)
}

export function OPEN_MCASH_PAYMENT_V2(
  config: {
   ...
   */
  const status$ = new BehaviorSubject<MCASHPaymentResult>({
    status: 'pending',
    popupWindowObject: PAY_WIN,
    form
  });
